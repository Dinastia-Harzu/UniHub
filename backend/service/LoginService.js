...
exports.loginPOST = function (body) {
    return new Promise(function (resolve, reject) {
      conexion.query(
        `SELECT correo, clave FROM usuario WHERE correo=${$(body.correo)} AND clave=${$(body.clave)}`,
        (err,filas) => {
          if (err) {
            console.error(err);
            ...
            if(filas.length == 0) {reject(responder(403,{motivo: "No existe este usuario"})); console.log(body.correo);}
                else {  resolve(responder(200, respuestas[200]));}0
